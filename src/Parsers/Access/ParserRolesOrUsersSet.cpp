...
namespace
{
    void collectQueryParameters(const ASTPtr & ast, Strings & query_parameter_names)
    {
        if (!ast)
            return;
        if (const auto * query_parameter = ast->as<ASTQueryParameter>())
        {
            query_parameter_names.emplace_back(query_parameter->name);
            return;
        }

        for (const auto & child : ast->children)
            collectQueryParameters(child, query_parameter_names);
    }
...
bool ParserRolesOrUsersSet::parseImpl(Pos & pos, ASTPtr & node, Expected & expected)
{
    ...
    Strings ignored_query_parameter_names;
    ...
    {
        for (auto & name : names)
            collectQueryParameters(name, ignored_query_parameter_names);
        names.clear();
    }
    ...
    result->ignored_query_parameter_names = std::move(ignored_query_parameter_names);
